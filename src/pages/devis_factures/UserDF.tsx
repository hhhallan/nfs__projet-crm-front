...
const UserDF: React.FC = () => {
	...
	useEffect(() => {
		...
		if (showDevis) {
			request = devisService.getByClient(user!.id)
		} else {
			request = factureService.getByClient(user!.id)
		}
		request.then((data) => {
			if (showDevis) {
	...
	return (
		<div className="page page-devis-factures">
			<div style={{display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'end', width: '100%'}}>
